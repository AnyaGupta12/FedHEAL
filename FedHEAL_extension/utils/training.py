...
def train(model: FederatedModel, private_dataset: FederatedDataset,
    ...
    for epoch_index in range(Epoch):
        ...
        # updateddddd
        # Adjust clients per round based on last round’s mean accuracy
        if epoch_index > 0 and hasattr(model, 'adjust_clients_per_round'):
            model.adjust_clients_per_round(mean_acc)
