...
class FederatedModel(nn.Module):
    ...
    #     # Initialize self.increase_history dict if not already
    #     if not hasattr(self, 'increase_history'):
    #         self.increase_history = {}

    #     # Initialize client entry if not present
    #     if client_id not in self.increase_history:
    #         self.increase_history[client_id] = {
    #             key: (updates[key] >= 0).float() for key in updates
    #         }
    #         return {key: torch.ones_like(val) for key, val in updates.items()}

    #     # Safeguard: ensure all keys exist in the client's increase_history
    #     for key in updates:
    #         if key not in self.increase_history[client_id]:
    ...
    def consistency_mask(self, client_id, update_diff):
        ...
        if client_id not in self.increase_history:
            self.increase_history[client_id] = {
                key: torch.zeros_like(val) for key, val in updates.items()
            }
            for key in updates:
                self.increase_history[client_id][key] = (updates[key] >= 0).float()
            return {key: torch.ones_like(val) for key, val in updates.items()}
        ...
        # Update increase history using running average
        for key in updates:
